...
main(int argc, char *argv[]){       
  13:	83 ec 18             	sub    $0x18,%esp
    pre=get_free_frame_cnt();
  16:	e8 8f 03 00 00       	call   3aa <get_free_frame_cnt>
  1b:	89 c6                	mov    %eax,%esi
  1d:	eb 0b                	jmp    2a <main+0x2a>
  1f:	90                   	nop
      pid=fork();
      if(pid==0){
        data[0]='a';        
  ...
  20:	78 20                	js     42 <main+0x42>
    for(n=0;n<64;n++){
  22:	83 c3 01             	add    $0x1,%ebx
  25:	83 fb 40             	cmp    $0x40,%ebx
  28:	74 60                	je     8a <main+0x8a>
      pid=fork();
  ...
  31:	75 ed                	jne    20 <main+0x20>
        data[0]='a';        
  33:	c7 05 c0 0a 00 00 61 	movl   $0x61,0xac0
  3a:	00 00 00 
        exit();
  3d:	e8 c0 02 00 00       	call   302 <exit>
        break;
    }
  ...
  43:	53                   	push   %ebx
    ...
    while(n--)wait();
  44:	8d 7b ff             	lea    -0x1(%ebx),%edi
    printf(1,"created %d child processes\n",n);
  47:	68 b8 07 00 00       	push   $0x7b8
  4c:	6a 01                	push   $0x1
  4e:	e8 0d 04 00 00       	call   460 <printf>
    post=get_free_frame_cnt();
  53:	e8 52 03 00 00       	call   3aa <get_free_frame_cnt>
    while(n--)wait();
  ...
  5d:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    while(n--)wait();
  60:	74 13                	je     75 <main+0x75>
  62:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  68:	83 ef 01             	sub    $0x1,%edi
  6b:	e8 9a 02 00 00       	call   30a <wait>
  70:	83 ff ff             	cmp    $0xffffffff,%edi
  73:	75 f3                	jne    68 <main+0x68>
    printf(1,"pre: %d, post: %d\n",pre,post);    
  75:	ff 75 e4             	pushl  -0x1c(%ebp)
  78:	56                   	push   %esi
  79:	68 d4 07 00 00       	push   $0x7d4
  7e:	6a 01                	push   $0x1
  80:	e8 db 03 00 00       	call   460 <printf>
    exit();
  85:	e8 78 02 00 00       	call   302 <exit>
    printf(1,"created %d child processes\n",n);
  8a:	50                   	push   %eax
  8b:	6a 40                	push   $0x40
    while(n--)wait();
  8d:	bf 3f 00 00 00       	mov    $0x3f,%edi
    printf(1,"created %d child processes\n",n);
  92:	68 b8 07 00 00       	push   $0x7b8
  97:	6a 01                	push   $0x1
  99:	e8 c2 03 00 00       	call   460 <printf>
    post=get_free_frame_cnt();
  9e:	e8 07 03 00 00       	call   3aa <get_free_frame_cnt>
  a3:	83 c4 10             	add    $0x10,%esp
  a6:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  a9:	eb bd                	jmp    68 <main+0x68>
  ab:	66 90                	xchg   %ax,%ax
  ad:	66 90                	xchg   %ax,%ax
  af:	90                   	nop
...
printf(int fd, char *fmt, ...)
{
 ...
 614:	e9 8f fe ff ff       	jmp    4a8 <printf+0x48>
          s = "(null)";
 619:	bb e7 07 00 00       	mov    $0x7e7,%ebx
        while(*s != 0){
